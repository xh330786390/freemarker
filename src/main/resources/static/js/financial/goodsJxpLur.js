...
function getParam() {
    ...
    $(".fina-box:checkbox:checked").each(function (i) {
        ...
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].goodsItem"] = row.find("td").eq(4).html();
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].supplierCode"] = row.find("td").eq(5).html();
        ...
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].invoiceAttribute"] = 1;
        ...
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].invoiceWeight"] = row.find("td").eq(11).find("input").val() * 10000;
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].invoiceAmount"] = row.find("td").eq(12).find("input").val() * 100;
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].taxRate"] = parseFloat(row.find("td").eq(13).find("input").val()) * 100;
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].invoiceTax"] = row.find("td").eq(14).find("input").val() * 100;
        inInvoiceVOS["inInvoiceItemVOS[" + i + "].invoiceTaxAmount"] = row.find("td").eq(15).find("input").val() * 100;
    ...
    if(second == 0){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                alert('录入进项票成功！')
                // window.location.href="/zall/chain/purchase/inInvoice/list";
