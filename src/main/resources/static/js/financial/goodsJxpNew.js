...
function checkParam() {
    ...
    if(businessType == 0 || (contractCode.length == 0 && supplierCode.length == 0 && supplierName == 0)){
...
function getParam() {
    ...
    param.supplierName = $("#supplierName").val().trim();
                ...
                if(data.inInvoiceVOS.length == 0){
                    ...
                    for(var i = 0; i < data.inInvoiceVOS.length; i++){
                        ...
                        str = '<tr><td><input type="checkbox" class="good-box"></td>\n' + '<td>' + businessType + '</td>' +
                            ...
                            '<td>'+ numUtils.floatWeight(data.inInvoiceVOS[i].putInStorageWeight) + '</td>' +
                            '<td>'+ numUtils.floatMoney(data.inInvoiceVOS[i].payAllMoney) + '</td>' +
                            '<td>'+ numUtils.floatWeight(data.inInvoiceVOS[i].invoiceWeight) + '</td>' +
                            '<td>'+ numUtils.floatMoney(data.inInvoiceVOS[i].invoiceAmount) + '</td>' +
                            '<td>'+ numUtils.floatMoney(data.inInvoiceVOS[i].payAllMoney - data.inInvoiceVOS[i].invoiceAmount) + '</td>' +
                        ...
                        $(".luRu-jxp").click(function(){
                            alert(contractCode)
                            insertGoodsInvoices(contractCode);
                        });
                        $("#invoiceBody").append(str);
                        goodBoxClick();
                    }
...
function insertGoodsInvoices(contractCode) {
    var contractStr = '';
    var i = 0;
    if(contractCode == null || contractCode == 'undefined' || contractCode.length == 0){
    ...
    if(contractStr.length > 0){
        window.location.href="/zall/chain/purchase/inInvoice/insertInvoiceItemInfo?contractStr=" + contractStr + "&invoiceAttribute=" + invoiceAttribute;
    }else{
        alert("暂无数据！")
    }
