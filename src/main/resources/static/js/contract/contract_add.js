        ...
        success:function(data){
            ...
            if (data != null || data.length != 0){
                $.each(data.FrameProtocolGatherVOs,function (i,frame) {
                    html+='<option value="'+frame.id+'">'+frame.frameCode+'</option>';
                });
            }
            $("#frameId").html(html);
...
function submitContract(id) {
    ...
    if(validateForm()){
        ...
        ajax('/zall/chain/purchase/contract/create','post',{
            data:contractJson,
            success:function (data) {
                if (data.status == '200'){
                    DialogGO('提交成功');
...
function save() {
    if (check_$($("#sellContractCode"))){
        var contractJson = getJson();
        createJson(contractJson,"auditStatus",4);
        ajax('/zall/chain/purchase/contract/create','post',{
            data:contractJson,
            success:function (data) {
                if (data.status == '200'){
...
function getJson() {
    ...
    if($("#businessType").val()=='2' && $("input[name=canInsteadTransport]:checked").val() == '1'){
    ...
    $("#itemsTable tr").each(function (i) {
        if (i!=0) {
            i = i - 1;
            if (check_d($(this).find("select[name=breedName]").val())){
                ...
                createJson(contractJson, 'contractPurchaseItems[' + i + '].weight', accMul(getString($(this).find("input[name=weight]").val()),10000));
                createJson(contractJson, 'contractPurchaseItems[' + i + '].unitPrice', accMul(getString($(this).find("input[name=unitPrice]").val()),100));
                createJson(contractJson, 'contractPurchaseItems[' + i + '].money', accMul(getString($(this).find("input[name=money]").val()), 100));
